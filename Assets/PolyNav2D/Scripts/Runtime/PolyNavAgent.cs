...
public class PolyNavAgent : MonoBehaviour{
	...
	///The max speed
	[Header("移动的最大速度")]public float maxSpeed            = 3.5f;
	///The object mass
	[Header("物体的质量")][Tooltip("质量越大，惯性越大")]public float mass                = 20;
	///The distance to stop at from the goal
	[Header("物体到达目标点的距离")]public float stoppingDistance    = 0.1f;
	///The distance to start slowing down
	[Header("设置物体从目标开始减速的距离")]public float slowingDistance     = 1;
	///The rate at which it will accelerate
	[Header("物体的加速速率")]public float accelerationRate    = 2;
	///The rate at which it will slow down
	[Header("物体在达到目标时减速的速率")]public float decelerationRate    = 2;
	...
	[Header("减速和物体回避的前瞻距离")][Tooltip("您可以将此值设置为0，以禁用障碍和代理向前查看并提高性能")]public float lookAheadDistance   = 1;
	///The avoidance radius of the agent. 0 for no avoidance	
	[Header("物体与其他物体的回避半径")]public float avoidRadius         = 0;
	///Custom center offset from original transform position.
	[Header("物体原始位置的偏移量")]public Vector2 centerOffset      = Vector2.zero;
	///Rotate transform as well?
	[Header("物体是否旋转")]public bool rotateTransform      = false;
	///Speed to rotate at moving direction.
	[Header("勾选旋转时的旋转速度")]public float rotateSpeed         = 350;
	///Should the agent repath? Disable for performance.
	[Header("地图改变是否重新评估路径")][Tooltip("如果启用，地图改变上将重新评估路径。为了性能，一般禁用掉")]public bool repath 				 = true;
	///Should the agent be forced restricted within valid areas? Disable for performance.
	[Header("是否将物体强制在有效区域")][Tooltip("如果启用，将强制代理位置在有效区域内。为了性能，一般禁用掉")]public bool restrict 			 = false;
	///Go to closer point if requested destination is invalid? Disable for performance.
	[Header("是否强制将物体移动到更接近目标位置")][Tooltip("如果请求的目标无效，将使物体移动到更接近目标的位置")]public bool closerPointOnInvalid = true;
	///Will debug the path (gizmos). Disable for performance.
	[Header("是否绘制路线")][Tooltip("如果选中，将绘制路径在scene视图里")]public bool debugPath            = true;
